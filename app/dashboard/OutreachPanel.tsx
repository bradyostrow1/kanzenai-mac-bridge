...
export function OutreachPanel() {
  ...
  if (!state) {
    return (
      <div className="border border-rule bg-bg-1 mb-6">
        ...
        <div className="p-4 text-ink-3 text-[13px]">Loading…</div>
      </div>
    );
  }
  ...
  return (
    <section className="border border-rule bg-bg-1 mb-6">
      <div className="px-4 py-2.5 border-b border-rule flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.18em] text-ink-3">
          {state.resendConfigured ? (
            <span className="text-emerald-800">Resend connected · from {state.fromAddress}</span>
          ) : (
            <span className="text-amber-800">⚠ Resend not configured — add RESEND_API_KEY to .env.local</span>
          )}
        ...
        {state.emails.map((e) => {
          ...
          return (
            <div key={e.id} className="p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`w-1.5 h-1.5 rounded-full ${sent ? "bg-emerald-700" : failed ? "bg-red-700" : "bg-[#525252]"}`} />
                    <span className="text-ink-0 font-semibold text-[13px]">{e.vendor}</span>
                    <span className="text-[10px] text-ink-3 uppercase tracking-wider">
                      {e.commission}
                    </span>
                  </div>
                  <div className="text-[11px] text-ink-2 mb-1">
                    To: <code className="text-ink-0">{e.to}</code>
                  </div>
                  <div className="text-[11px] text-ink-3">{e.subject}</div>
                  {sent && (
                  ...
                  {failed && (
                    <div className="mt-2 text-[10px] text-red-700 inline-flex items-start gap-1">
                ...
                <div className="flex flex-col gap-1.5 items-end">
                  <button
                    ...
                    className="px-2.5 py-1 border border-rule hover:border-ink-2 transition text-[11px]"
                  >
              ...
              {isExpanded && (
                <div className="mt-3 p-3 bg-bg-2 border border-rule text-[11px] text-ink-0 whitespace-pre-wrap leading-relaxed">
                  {e.body}
                </div>
              )}
            </div>
      ...
      {!state.resendConfigured && (
        <div className="px-4 py-3 border-t border-rule bg-amber-950/20 text-[11px] text-amber-800 leading-relaxed">
